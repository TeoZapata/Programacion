...
def generar_reporte_historial_pdf(registros, fecha_desde=None, fecha_hasta=None,
                                  trabajador=None, output_dir=None):
    if output_dir is None:
        output_dir = Path.home() / "GeoInventario" / "reportes"
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = Path(output_dir) / f"Reporte_Historial_Herramientas_{ts}.pdf"
    ...
    story = [
        Paragraph("GEOINVENTARIO", styles["TituloGeo"]),
        Paragraph("REPORTE DE HISTORIAL DE HERRAMIENTAS", styles["SubtituloGeo"]),
        Paragraph(
    ...
    filtros = []
    if fecha_desde:
        filtros.append(f"Desde: {fecha_desde.strftime('%d/%m/%Y')}")
    if fecha_hasta:
        filtros.append(f"Hasta: {fecha_hasta.strftime('%d/%m/%Y')}")
    if trabajador:
        filtros.append(f"Trabajador: {trabajador}")
    if not filtros:
        filtros.append("Sin filtros")
    story.append(Paragraph(" | ".join(filtros), styles["CuerpoGeo"]))
    story.append(Spacer(1, 0.4 * cm))

    resumen = [
        ["Total movimientos", str(len(registros))],
        ["Asignaciones", str(sum(1 for r in registros if r.tipo_movimiento == "asignacion"))],
        ["Devoluciones", str(sum(1 for r in registros if r.tipo_movimiento == "devolucion"))],
        ["Mantenimientos", str(sum(1 for r in registros if r.tipo_movimiento == "mantenimiento"))],
    ]
    resumen_tabla = Table(resumen, colWidths=[5.5 * cm, 3.2 * cm])
    resumen_tabla.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), HexColor("#F7FAFC")),
        ("TEXTCOLOR", (0, 0), (-1, -1), HexColor("#1E3A5F")),
    ...
    data = [[
        "Fecha", "Herramienta", "Tipo", "Trabajador", "Estado", "Observaciones"
    ]]
    for registro in registros:
        herramienta = registro.herramienta.nombre if registro.herramienta else "—"
        trabajador_nombre = registro.trabajador.nombre if registro.trabajador else "—"
        estado = f"{registro.estado_anterior or '—'} -> {registro.estado_nuevo or '—'}"
        data.append([
            registro.fecha_evento.strftime("%d/%m/%Y %H:%M") if registro.fecha_evento else "—",
            herramienta,
            (registro.tipo_movimiento or "—").replace("_", " ").title(),
            trabajador_nombre,
            estado,
            registro.observaciones or "—",
        ])

    tabla = Table(data, repeatRows=1, colWidths=[2.5 * cm, 4.0 * cm, 2.4 * cm, 3.0 * cm, 3.2 * cm, 4.8 * cm])
