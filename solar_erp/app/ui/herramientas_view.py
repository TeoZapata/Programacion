...
from PySide6.QtWidgets import (
    ...
    QProgressBar, QSizePolicy, QAbstractItemView, QDateEdit
)
...
from app.reports.reporte_herramientas import (
    generar_reporte_general,
    generar_reporte_historial_pdf,
    generar_reporte_trabajador,
)
...
class _CargarWorker(QThread):
    ...
    listo = Signal(list, list, list)  # (herramientas, asignaciones, historial)
    error = Signal(str)
    ...
    def run(self):
        ...
        try:
            ...
            historial = svc.historial_general()
            self.listo.emit(herramientas, asignaciones, historial)
        except Exception as e:
...
class HerramientasView(QWidget):
    def __init__(self, usuario=None):
        ...
        self._busqueda_inv_pendiente = ""
        self._timer_busqueda_inv = QTimer(self)
        self._timer_busqueda_inv.setSingleShot(True)
        self._timer_busqueda_inv.setInterval(180)
        self._timer_busqueda_inv.timeout.connect(self._aplicar_filtro_inventario)
        self._build_ui()
        self._cargar_todo()
    ...
    def _build_ui(self):
        ...
        self.tabs.addTab(self._build_tab_historial(), "📚  Historial")
    ...
    def _build_tab_inventario(self):
        ...
        self.inp_buscar_inv.setStyleSheet("QLineEdit{background:#F0F4F8;color:#1A1A2E;"
            ...
            "QLineEdit:focus{border:2px solid #2ECC71;background:white;}")
        self.inp_buscar_inv.textChanged.connect(self._programar_busqueda_inventario)
        ...
        self.cmb_filtro_estado.currentIndexChanged.connect(self._aplicar_filtro_inventario)
    ...
    def _build_tab_historial(self):
        ...
        self.cmb_hist_trabajador = QComboBox()
        self.cmb_hist_trabajador.setMinimumHeight(38)
        self.cmb_hist_trabajador.addItem("Todos los trabajadores", None)
        self.cmb_hist_trabajador.currentIndexChanged.connect(self._filtrar_historial)
        toolbar.addWidget(self.cmb_hist_trabajador)

        self.fecha_hist_desde = QDateEdit()
        self.fecha_hist_desde.setCalendarPopup(True)
        self.fecha_hist_desde.setDisplayFormat("dd/MM/yyyy")
        self.fecha_hist_desde.setDate(QDate.currentDate().addMonths(-1))
        self.fecha_hist_desde.dateChanged.connect(self._filtrar_historial)
        toolbar.addWidget(self.fecha_hist_desde)

        self.fecha_hist_hasta = QDateEdit()
        self.fecha_hist_hasta.setCalendarPopup(True)
        self.fecha_hist_hasta.setDisplayFormat("dd/MM/yyyy")
        self.fecha_hist_hasta.setDate(QDate.currentDate())
        self.fecha_hist_hasta.dateChanged.connect(self._filtrar_historial)
        toolbar.addWidget(self.fecha_hist_hasta)

        self.inp_buscar_hist = QLineEdit()
        self.inp_buscar_hist.setPlaceholderText("🔍  Buscar por herramienta, movimiento u observación...")
        self.inp_buscar_hist.setMinimumWidth(280)
        self.inp_buscar_hist.textChanged.connect(self._filtrar_historial)
        toolbar.addWidget(self.inp_buscar_hist)

        toolbar.addStretch()

        btn_hist_pdf = QPushButton("📄 Exportar Historial PDF")
        btn_hist_pdf.setObjectName("btn_primary")
        btn_hist_pdf.setMinimumHeight(38)
        btn_hist_pdf.clicked.connect(self._exportar_historial_pdf)
        toolbar.addWidget(btn_hist_pdf)

        lay.addLayout(toolbar)

        self.tabla_hist = QTableWidget(0, 7)
        self.tabla_hist.setHorizontalHeaderLabels([
            "Fecha", "Herramienta", "Movimiento", "Trabajador", "Estado anterior", "Estado nuevo", "Observaciones"
        ])
        configurar_tabla(
            self.tabla_hist,
            col_stretch=6,
            cols_fijas=[(0, 125), (1, 180), (2, 120), (3, 170), (4, 110), (5, 110)],
        )
        ...
        self.tabla_hist.setSelectionBehavior(QAbstractItemView.SelectRows)
    ...
    def _on_carga_lista(self, herramientas, asignaciones, historial):
        self._herramientas = herramientas
        self._asignaciones = asignaciones
        self._historial = historial
        self._cargar_filtro_trabajadores_historial()
        self._aplicar_filtro_inventario()
    ...
    def _cargar_filtro_trabajadores_historial(self):
        actual = self.cmb_hist_trabajador.currentData() if hasattr(self, "cmb_hist_trabajador") else None
        self.cmb_hist_trabajador.blockSignals(True)
        self.cmb_hist_trabajador.clear()
        self.cmb_hist_trabajador.addItem("Todos los trabajadores", None)
        agregados = set()
        for registro in self._historial:
            trabajador = registro.trabajador
            if trabajador and trabajador.id not in agregados:
                self.cmb_hist_trabajador.addItem(trabajador.nombre, trabajador.id)
                agregados.add(trabajador.id)
        idx = self.cmb_hist_trabajador.findData(actual)
        self.cmb_hist_trabajador.setCurrentIndex(idx if idx >= 0 else 0)
        self.cmb_hist_trabajador.blockSignals(False)
    ...
    def _poblar_historial(self, registros):
        self.tabla_hist.setRowCount(0)
        for registro in registros:
            r = self.tabla_hist.rowCount()
            self.tabla_hist.insertRow(r)
            fecha = registro.fecha_evento.strftime("%d/%m/%Y %H:%M") if registro.fecha_evento else "-"
            herramienta = registro.herramienta.nombre if registro.herramienta else "-"
            movimiento = (registro.tipo_movimiento or "-").replace("_", " ").title()
            trabajador = registro.trabajador.nombre if registro.trabajador else "-"

            self.tabla_hist.setItem(r, 0, _item(fecha))
            self.tabla_hist.setItem(r, 1, _item(herramienta))
            self.tabla_hist.setItem(r, 2, _item(movimiento))
            self.tabla_hist.setItem(r, 3, _item(trabajador))
            self.tabla_hist.setItem(r, 4, _item(registro.estado_anterior))
            self.tabla_hist.setItem(r, 5, _item(registro.estado_nuevo))
            self.tabla_hist.setItem(r, 6, _item(registro.observaciones))
            self.tabla_hist.setRowHeight(r, 36)

    # ── Búsqueda / filtros ────────────────────────────────────────────────────
    def _programar_busqueda_inventario(self, texto):
        self._busqueda_inv_pendiente = (texto or "").strip().lower()
        self._timer_busqueda_inv.start()

    def _aplicar_filtro_inventario(self):
        texto = (self._busqueda_inv_pendiente or self.inp_buscar_inv.text() or "").strip().lower()
        estado = self.cmb_filtro_estado.currentText()
        herr = self._herramientas
        if texto:
            herr = [
                h for h in herr
                if texto in (h.nombre or "").lower()
                or texto in (h.codigo or "").lower()
                or texto in (h.categoria or "").lower()
                or texto in (h.marca or "").lower()
                or texto in (h.modelo or "").lower()
                or texto in (h.numero_serie or "").lower()
                or texto in (h.ubicacion or "").lower()
            ]
    ...
    def _filtrar_historial(self):
        texto = self.inp_buscar_hist.text().strip().lower()
        trabajador_id = self.cmb_hist_trabajador.currentData()
        fecha_desde = self.fecha_hist_desde.date().toPython()
        fecha_hasta = self.fecha_hist_hasta.date().toPython()

        registros = []
        for registro in self._historial:
            fecha_evento = registro.fecha_evento.date() if registro.fecha_evento else None
            if fecha_evento and fecha_evento < fecha_desde:
                continue
            if fecha_evento and fecha_evento > fecha_hasta:
                continue
            if trabajador_id and registro.trabajador_id != trabajador_id:
                continue
            if texto:
                contenido = " ".join([
                    registro.herramienta.nombre if registro.herramienta else "",
                    registro.trabajador.nombre if registro.trabajador else "",
        ...
        self._historial_filtrado = registros
        self._poblar_historial(registros)
    ...
    def _exportar_historial_pdf(self):
        registros = list(self._historial_filtrado)
        if not registros:
            QMessageBox.information(self, "Sin datos", "No hay movimientos en el historial para exportar.")
            return

        try:
            trabajador = self.cmb_hist_trabajador.currentText()
            if self.cmb_hist_trabajador.currentData() is None:
                trabajador = None
            ruta = generar_reporte_historial_pdf(
                registros=registros,
                fecha_desde=self.fecha_hist_desde.date().toPython(),
                fecha_hasta=self.fecha_hist_hasta.date().toPython(),
            ...
            QMessageBox.critical(self, "Error al generar historial", str(e))
