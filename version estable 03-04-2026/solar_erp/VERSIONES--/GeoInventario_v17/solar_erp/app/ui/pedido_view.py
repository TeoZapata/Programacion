...
class PedidoMaterialView(QWidget):
    ...
    def _filtrar_tabla(self, texto):
        ...
        if not texto:
            ...
            self._poblar_tabla(self._materiales_db)
            return
        filtrados = [m for m in self._materiales_db
                     if texto in m.nombre_material.lower()
                     or (m.categoria and texto in m.categoria.lower())]
        self._poblar_tabla(filtrados)
        self.banner.setVisible(len(filtrados) == 0)
    ...
    def _agregar_externo(self):
        ...
        if dlg.guardar_en_bd:
            ...
            try:
                ...
                db.close()
                self._materiales_db = InventarioService(SessionLocal()).listar()
                self._poblar_tabla(self._materiales_db)
